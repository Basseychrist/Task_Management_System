...
const validateTask = [
  ...
  body('dueDate')
    ...
    .isISO8601().toDate().withMessage('Invalid date format for Due Date')
    .custom((value) => {
      if (value) {
  ...
  body('tags')
    ...
    .custom((value) => {
      if (value) {
        ...
        if (!tagsArray.every(tag => typeof tag === 'string' && tag.length > 0)) {
          throw new Error('Tags must be a comma-separated list of non-empty strings');
        }
      }
      return true;
    }),
    ...
    .custom((value) => {
        try {
            if (value) {
                ...
                if (!Array.isArray(attachmentsArray) || !attachmentsArray.every(att => att.filename && att.url)) {
                    throw new Error('Attachments must be a JSON array of objects with filename and url');
                }
            }
        } catch (e) {
